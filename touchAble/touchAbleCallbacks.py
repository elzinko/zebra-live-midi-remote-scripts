...
class touchAbleCallbacks:
    ...
    def convertToSimpler(self, msg):
        ...
        self.oscServer.sendOSC("/NSLOG_REPLACE", ("midi to simpler try"))

        Live.Conversions.create_midi_track_with_simpler(LiveUtils.getSong(), clip)

        self.oscServer.sendOSC("/NSLOG_REPLACE", ("midi to simpler ready"))
    ...
    def update_clip_waveform(self, clip, tid, cid, start_in_percent, end_in_percent):
        try:
        #self.oscServer.sendOSC("/NSLOG_REPLACE", ("update_simpler_waveform"))

            path = clip.file_path
    
            reset_data = [tid, cid, repr2(path), start_in_percent, end_in_percent]
        ...
        if clip != None:
            ...
            if clip.is_audio_clip:
                ...
                try:
                    ...
                    self.touchAble.clip_gain_changed(clip,trackNumber,clipNumber)
                    self.touchAble.clip_gain_str_changed(clip,trackNumber,clipNumber)
                    self.touchAble.clip_coarse_changed(clip,trackNumber,clipNumber)
                    self.touchAble.clip_fine_changed(clip,trackNumber,clipNumber)
                except:
                    pass
            ...
            if clip.is_audio_clip:
                ...
                reset_data = [int(trackNumber), int(clipNumber), repr2(path), float(0.0), float(1.0)]
    ...
    def updateWaveformAIF(self,tid,cid,clip):

            path = clip.file_path
            x = repr2(clip.file_path)
            
            #if os.path.isfile(x):
            #self.oscServer.sendOSC("/NSLOG_REPLACE", "file exists")


            a_message = "audio clip path: " + x


            song =  aifc.open(repr2(path), 'rb')

            length = song.getnframes()
            channels = song.getnchannels()
            samplewidth = song.getsampwidth()
            framerate = song.getframerate()
            
            """for aa in dir(clip.view):
                self.oscServer.sendOSC("/NSLOG_REPLACE", str(aa))
            ...
            waveform_data = [int(tid)]
            waveform_data.append(int(cid))
            a_sample_length = str(clip.sample_length);
            ...
            self.oscServer.sendOSC("/clip/reset_waveform", (tuple(waveform_data)))



    def sign_extend(self,bits, value):
        """ Sign-extend from http://stackoverflow.com/questions/32030412/twos-complement-sign-extension-python """
        sign_bit = 1 << (bits - 1)
        return (value & (sign_bit - 1)) - (value & sign_bit)


    def get_samples(self,w):
        w.rewind()
    ...
    def updateWaveformWAV(self,tid,cid,clip):
        ...
        """self.oscServer.sendOSC("/NSLOG_REPLACE", "opened file file length " + str(length) + "bytes per sample " + str(sampwidth) + "num channels " + str(dc) + " " + fmt);"""

        x = 0
        count = 0

        max_send_length = self.touchAble.maxSampleLength
        addition = (length/float(max_send_length))
        kkkk = 0
        ...
        while x < length:
            ...
            waveFile.setpos(kkkk)

            iframe = waveFile.readframes(1)

            value = 1.
            if channels == 2:
                (iframe,iframer) = struct.unpack(fmt, iframe)
            else:
                iframe = struct.unpack(fmt, iframe)[0]

            iframe -= dc
            value = int(iframe)
            value = (value - 32768)/32768. + 1.


            """self.oscServer.sendOSC("/NSLOG_REPLACE", str(value))"""
            ...
            if count == 128:
                self.oscServer.sendOSC("/clip/update_waveform", (tuple(waveform_data)))
                count = 0
                waveform_data = [int(tid)]
                waveform_data.append(int(cid))
                    
        self.oscServer.sendOSC("/clip/update_waveform", (tuple(waveform_data)))
        self.oscServer.sendOSC("/clip/updated_waveform", "1")
        ...
        """for i in range(waveFile.getnframes()):
            iframe = waveFile.readframes(1)
            value = 1.
            (ileft,iframe) = struct.unpack(fmt, iframe)
            value = int(iframe)
            iframe -= dc
            self.oscServer.sendOSC("/NSLOG_REPLACE", str(value)) """
    ...
    def clipClearAutomationMixer(self, msg):
        ...
        """ for obj_string in dir(live_track.mixer_device):
            self.oscServer.sendOSC("/NSLOG_REPLACE", repr2(obj_string)) """

        if parameter_inx == 0:
            parameter = clip.muted
            self.oscServer.sendOSC("/NSLOG_REPLACE", "got muted " + repr2(parameter))
            ...
            self.oscServer.sendOSC("/NSLOG_REPLACE", "got crossfader assign " + repr2(parameter))
        elif parameter_inx > 3:
...
#        self.liveOSC.log_message( "*** SELECTION_CB: " + str( msg[ 2 ] ) + ", " + str( msg[ 3 ] ) + ", " + str( msg[ 4 ] ) + ", " + str( msg[ 5 ] )  )
    ...
    def songtime_change(self):
        self.oscServer.sendOSC("/set/playing_position", (LiveUtils.getSong().current_song_time + 0.0001,float(4),float(4)))
    ...
    def mastermeters(self):
        ...
        if vall != self.mmcache:
            ...
            if vall == 0:
                self.oscServer.sendOSC('/master/meter',(0.0000000001,0.0000000001))
            self.mmcache = vall
        i = 0
        for track in tracks:
            vall = track.output_meter_left
    ...
    def expand_device_dev(self, device):
        #self.oscServer.sendOSC("/NSLOG_REPLACE", ("expanding:", repr2(device.class_name)))
    ...
    def tracksCB(self, msg):
        ...
        self.oscServer.sendOSC("/NSLOG_REPLACE", "set size tracks cb")
        ...
        for track in LiveUtils.getTracks():
            ...
            self.oscServer.sendOSC("/track", (trackNumber, repr2(nm), col, grouptrack, int(is_midi_track),live_pointer))
        ...
        self.loadBrowserStartUp(load_browser,a)
        ...
        self.touchAble.loading = 0
        self.oscServer.sendOSCUDP("/NSLOG_REPLACE", ("loading2 = ", self.touchAble.loading))
        if self.touchAble.should_check_if_load == 1:
            self.oscServer.sendOSCUDP("/NSLOG_REPLACE", ("should_check_if_load2 = ", self.touchAble.should_check_if_load))
            ...
            if (trackTotal != trackTotal2 or sceneTotal != sceneTotal2 or returnsTotal != returnsTotal2):
                self.oscServer.sendOSCUDP("/NSLOG_REPLACE", ("load is needed"))

                self.touchAble.should_check_if_load = 0
                self.touchAble.tracksCB(msg)
    ...
    def loadBrowserStartUp(self,load_browser,a):
    ...
    def my_random_string(self, string_length=12):
    ...
    def sendTrackIO(self, trackNumber, track, type = "", track_type = 0):
        ...
        if type == "input_routing_type" or type == "":
            ...
            io_data.append(repr2(track_input_route_type_name))
            found_port = 0
        ...
        if type == "input_routing_channel" or type == "":
            ...
            io_data.append(repr2(track_input_routing_channel_name))
            found_port = 0
        ...
        if type == "output_routing_type" or type == "":
            ...
            io_data.append(repr2(track_output_route_type_name))
            found_port = 0
        ...
        if type == "output_routing_channel" or type == "":
            ...
            io_data.append(repr2(track_output_routing_channel_name))
            found_port = 0
        ...
        if type == "available_input_routing_channels" or type == "":
            ...
            for i in range(len(track.available_input_routing_channels)):
                ...
                io_data.append(repr2(route_name))
            if type != "":
                self.oscServer.sendOSC("/track/io", io_data)
    ...
    def sendTrackClips(self, trackNumber, track):
        ...
        for clipSlot in track.clip_slots:
            if clipSlot.clip != None:
                ...
                self.oscServer.sendOSC("/clip", (trackNumber, clipNumber, repr2(nm), clip.color, play,is_audio_clip))
    ...
    def nameTrackCB(self, msg):
        ...
        self.oscServer.sendOSC("/NSLOG_REPLACE", "name track")
    ...
    def nameClipCB(self, msg):
            ...
            for track in LiveUtils.getTracks():
                for clipSlot in track.clip_slots:
                    if clipSlot.clip != None:
                        ...
                        self.oscServer.sendOSC("/clip", (trackNumber, clipNumber, repr2(nm), clipSlot.clip.color, play,int(0)))
                        ...
                        if clipSlot.has_stop_button == 1:
                            if LiveUtils.getSong().tracks[trackNumber].is_foldable != 1:
                                ...
                                self.oscServer.sendOSC("/clip", (trackNumber, clipNumber,"12>>3", 3289650, play,int(0)))
        ...
        if len(msg) == 4:
            trackNumber = msg[2]
            clipNumber = msg[3]
            self.oscServer.sendOSC("/clip", (trackNumber, clipNumber, repr2(LiveUtils.getClip(trackNumber, clipNumber).name), LiveUtils.getClip(trackNumber, clipNumber).color,int(0),int(0)))
            return
        #renaming a clip
        if len(msg) == 5:
            trackNumber = msg[2]
    ...
    def songpsCB(self, msg):
        ...
        songtime = LiveUtils.getSong().current_song_time
        self.oscServer.sendOSC("/set/playing_position", (float(songtime) + 0.001,float(4),float(4)))
    ...
    def deviceCB(self, msg):
        ...
        if len(msg) == 5: 
            ...
            for i in range(len(params)):
                ...
                po.append(int(params[i].is_enabled))
                po.append(int(params[i].automation_state))
    ...
    def devCB(self, msg):
        ...
        for track in LiveUtils.getTracks():
            ...
            for i in range(len(devices)):
                ...
                #self.oscServer.sendOSC("/NSLOG_REPLACE", "pointer device start")
                #self.oscServer.sendOSC("/NSLOG_REPLACE", "pointer start" + str(device._live_ptr))
                """self.oscServer.sendOSC("/track/device", (0, trackNr, i, nm, onoff, num, numParam, cnam, live_pointer))"""
                 #wait 0.01 seconds
            trackNr = trackNr +1
    ...
    def send_update_for_device(self, device, track, tid, did, type, num, number_of_steps, indices, i):
        ...
        try:
            ...
            self.oscServer.sendOSC("/NSLOG_REPLACE", "failed expanding:" + repr2(device.name))
            pass
        ...
        #self.oscServer.sendOSC("/NSLOG_REPLACE", "send device parameters at start up")
        #self.oscServer.sendOSC("/NSLOG_REPLACE", "update device parameter")

        self.touchAble.update_simpler_parameters(device, track, tid, did, type , number_of_steps, indices)
        
        a_version = Live.Application.get_application().get_major_version()
        
        if a_version >= 10:
            self.touchAble.update_compressor_parameters(device, track, tid, did, type , number_of_steps, indices)
            self.touchAble.update_wavetable_parameters(device, track, tid, did, type , number_of_steps, indices)
